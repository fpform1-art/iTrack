...
export function MobileTopBar() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-12 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur sm:hidden">
      <Link href="/home" className="text-base font-semibold text-slate-900">
        iTrack
      </Link>
      <Link
        href="/settings"
        className="flex h-7 w-7 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-600"
      >
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 border-t border-slate-200 bg-white sm:hidden">
      ...
      <button
        ...
        className="flex flex-col items-center justify-center gap-0.5 py-2 text-slate-900"
      >
        <span className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-lg font-medium text-white">
          +
        </span>
